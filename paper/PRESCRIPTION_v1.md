# 处方手册 v1.3(v1.2→v1.3 依拍板1 2026-07-20:规则 5 加契约前置条款)
...
5. **决策类修复三档**(v1.3):**前置条款——steering 一线仅适用于模型已遵循
   目标体裁契约的场合;契约缺席时,先用最小 SFT 装契约、再上 steering**
   (Llama 实测:契约缺席,方向推不出契约 .058)。契约在位时:数据层存在
