...
for g in $(seq 0 $((NGPU-1))); do
  CUDA_VISIBLE_DEVICES="$g" python3 probes/run_probes.py --model "$MODEL" --types P,R,S --outdir probes/out2 --shard "${g}:${NGPU}" > "/tmp/fix_prs_${g}.log" 2>&1 &
done; wait
for g in $(seq 0 $((NGPU-1))); do
  CUDA_VISIBLE_DEVICES="$g" python3 probes/run_probes.py --model "$MODEL" --types W2 --ids-file probes/data/w2_rerun_ids.json --outdir probes/out2 --shard "${g}:${NGPU}" > "/tmp/fix_w2_${g}.log" 2>&1 &
done; wait
